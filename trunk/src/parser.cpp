...
static void parser_start_element(void *userdata, const XML_Char *name,
	...
	if (! parser->aegis_element_found) {
		if ( ! strcmp((char *) name, "aegis")) {
			...
			if (version) {
				if (strcmp(version, VERSION) < 0)
					parser->warning("detected an old configuration file "
							"version %s. aegis version is " VERSION
							"\n", version);
				else if (strcmp(version, VERSION) > 0)
					parser->warning("detected an old version of aegis "
							VERSION ". configuration file version is "
							"%s\n", version);
			}
			parser->aegis_element_found = true;
			delete attr_tst;
			return;
		}
	}
...
static void parser_start_doctype_decl(void *userdata,
	...
	if (strcmp((char *) doctypename, "aegis") != 0)
...
bool CfgElementHandler::startElement(ParserElement element,
	...
	switch (element) {
		...
		case ParserElementDo: {
								  ...
								  parser->ws->aegis->eh->doings.push_back(doing);
							  } break;
										...
										if (value) {
											...
											if (value) {
												list<ModifierMap *>::iterator it =
													parser->ws->aegis->modmaps.begin();
												for (; it != parser->ws->aegis->modmaps.end(); it++) {
													if (! strcasecmp(value, (*it)->name)) {
														*x11_modifier_value |= (*it)->modifier;
														break;
													}
												} if (it == parser->ws->aegis->modmaps.end())
