...
void EventHandler::eventLoop(set<int> *return_mask, XEvent *event) {
	...
	for (;;) {
		if (XPending(aegis->display)) {
			XNextEvent(aegis->display, event);
			if (return_mask->find(event->type) != return_mask->end()) return;
			handleEvent(event);
		} else if (aegis->quit_signal) {
			quit(EXIT_SUCCESS);
		} else if (aegis->restart_signal) {
			restart(NULL);
		} else if (aegis->unknown_signal) {
			quit(EXIT_FAILURE);
		} else if (aegis->timer->timer_signal) {
			aegis->timer->handleTimeout();
			aegis->timer->timer_signal = 0;
			...
			if (cr == NULL) {
				...
				cairo_set_target_drawable(cr, aegis->display,
						DefaultRootWindow(aegis->display));
			}
...
void EventHandler::handleEvent(XEvent *event) {
	...
	switch (event->type) {
		...
		case ButtonPress:
			ed.type = ButtonPress;
			if (aegis->double_click) {
				if (! aegis->timer->exitsInterrupt(DOUBLECLICK_TIMER_ID)) {
					Action *double_click_action = new Action;
					double_click_action->delay = aegis->double_click;
		...
		case MapRequest: {
							 ...
							 if (wo) {
								 XQueryPointer(aegis->display, event->xmaprequest.parent,
...
void EventHandler::evFocus(XFocusChangeEvent *e) {
	...
	if (e->type == FocusIn && e->window != focused) {
		if ((wo = aegis->findWin(e->window,
			...
			aegis->addToFocusHistory(e->window);

			newfocus = true;
			XQueryPointer(aegis->display, ws->id, &w, &w, &rx, &ry, &i, &i,
					&(ed.x11mod));
		...
		if ((wo = aegis->findWin(focused,
			...
			if (! newfocus)
				XQueryPointer(aegis->display, ws->id, &w, &w, &rx, &ry, &i,
						&i, &(ed.x11mod));
	...
	if ((ww = (WaWindow *) aegis->findWin(e->window, WindowType))) {
		...
		if (status && attr.screen && (! attr.override_redirect)) {
			if (ws->net->isSystrayWindow(e->window)) {
				if (! aegis->findWin(e->window, SystrayType)) {
					XSelectInput(ws->display, e->window,
							StructureNotifyMask);
					SystrayWindow *stw = new SystrayWindow(e->window, ws);
					aegis->window_table.insert(make_pair(e->window, stw));
...
void EventHandler::evUnmapDestroy(XEvent *e) {
	...
	if ((wo = aegis->findWin((e->type == UnmapNotify)?
					e->xunmap.window:
		...
		if (wo->type == WindowType) {
			...
			aegis->window_table.erase(stw->id);
...
void EventHandler::evClientMessage(XEvent *e, EventDetail *ed) {
	...
	else if (e->xclient.message_type == aegis->net->aegis_net_restart) {
		restart(NULL);
	}
	else if (e->xclient.message_type == aegis->net->aegis_net_shutdown) {
		quit(EXIT_SUCCESS);
	}
			...
			for (int i = 1; i < 3; i++) {
				if ((unsigned long) e->xclient.data.l[i] ==
					...
					switch (e->xclient.data.l[0]) {
						...
						aegis->net->net_wm_maximized_vert ||
						(unsigned long) e->xclient.data.l[i] ==
						aegis->net->net_wm_maximized_horz) {
					if (max_done) break;
					max_done = true;
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->net_wm_state_above ||
						(unsigned long) e->xclient.data.l[i] ==
						aegis->net->net_wm_state_stays_on_top) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->net_wm_state_below ||
						(unsigned long) e->xclient.data.l[i] ==
						aegis->net->net_wm_state_stays_at_bottom) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->net_wm_state_skip_taskbar) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->net_wm_state_fullscreen) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
							ww->fullscreenOff();
							ww->unMaximize();
						...
						aegis->net->aegis_net_wm_state_decor) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->aegis_net_wm_state_decortitle) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->aegis_net_wm_state_decorborder) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
						...
						aegis->net->aegis_net_wm_state_decorhandles) {
					switch (e->xclient.data.l[0]) {
						case _NET_WM_STATE_REMOVE:
	...
	else if (e->xclient.message_type == aegis->net->xdndenter ||
			e->xclient.message_type == aegis->net->xdndleave) {
		if (e->xclient.message_type == aegis->net->xdndenter) {
			e->type = EnterNotify;
			ed->type = EnterNotify;
		} else {
			e->type = LeaveNotify;
...
void EventHandler::evAct(XEvent *e, Window win, EventDetail *ed) {
	...
	if ((it = aegis->window_table.find(win)) !=
		...
		if (awo->type == WindowType) {
			if (! status) {
				XAllowEvents(aegis->display, ReplayPointer, CurrentTime);
				XAllowEvents(aegis->display, ReplayKeyboard, CurrentTime);
			} else {
				XAllowEvents(aegis->display, AsyncPointer, CurrentTime);
				XAllowEvents(aegis->display, AsyncKeyboard, CurrentTime);
			}
		}
