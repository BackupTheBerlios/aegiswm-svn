...
void Aegis::getModifierMappings(void) {
	while (! modmaps.empty()) {
...
WindowObject *Aegis::findWin(Window id, long int mask) {
...
bool Aegis::focusNew(Window win, bool make_vis) {
	bool status = false;
	map<Window, WindowObject *>::iterator it;
...
void Aegis::addToFocusHistory(Window win) {
	if (win == None) return;
	focus_history.remove(win);
	focus_history.push_front(win);
}

void Aegis::removeFromFocusHistory(Window win) {
	if (win == None) return;
	focus_history.remove(win);
}
...
bool validate_drawable(Drawable d, unsigned int *w, unsigned int *h) {
	...
	XSync(aegis->display, false);

	XEvent e;
	if (XCheckTypedWindowEvent(aegis->display, d, DestroyNotify, &e)) {
		XPutBackEvent(aegis->display, &e);
		return false;
	}
	...
	hush = 1;
	if (w && h)
		XGetGeometry(aegis->display, d, &_wd, &_d, &_d, w, h, &_ud, &_ud);
	else
		XGetGeometry(aegis->display, d, &_wd, &_d, &_d, &_ud, &_ud, &_ud,
				&_ud);
...
const bool validate_window_mapped(Window id) {
	...
	XEvent e;
	if (XCheckTypedWindowEvent(aegis->display, id, DestroyNotify, &e) ||
			XCheckTypedWindowEvent(aegis->display, id, UnmapNotify, &e)) {
		XPutBackEvent(aegis->display, &e);
		return false;
	}
	return true;
...
int xerrorhandler(Display *d, XErrorEvent *e) {
	...
	if (! hush) {
		...
		if ((it = aegis->window_table.find(e->resourceid)) !=
...
void signalhandler(int sig) {
	...
	switch (sig) {
		case SIGCHLD:
			...
			break;
		case SIGINT:
			aegis->quit_signal = true;
			break;
		case SIGHUP:
			aegis->restart_signal = true;
			break;
		default:
			aegis->unknown_signal = true;
			break;
	}
}
...
void restart(char *command) {
	...
	cme.display = aegis->display;
	cme.format = 32;
	cme.window = aegis->wascreen_list.front()->id;
	cme.data.l[0] = aegis->wascreen_list.front()->id;
...
again:
	...
	err = iconv(aegis->utf8conv, (ICONV_CONST char **) &p,
