...
AegisWindow::~AegisWindow(void) {
	...
	aegis->removeFromFocusHistory(id);
...
void AegisWindow::redrawWindow(bool force_if_viewable) {
	...
	if (resizew || resizeh) {
		...
		XMoveResizeWindow(display, frame->id, frame->attrib.x,
				frame->attrib.y, frame->attrib.width,
				frame->attrib.height);
...
void AegisWindow::move(XEvent *e) {
	...
	DWindowObject *dwo = (DWindowObject *)
		aegis->findWin(e->xany.window, ANY_DECOR_WINDOW_TYPE);
	...
	if (XGrabPointer(display, grab_window, true, ButtonReleaseMask |
		...
		move_resize = false;
		aegis->eh->move_resize = EndMoveResizeType;
		setWindowState(wstate & ~StateMoveMask);
	...
	for (;;) {
		aegis->eh->eventLoop(&aegis->eh->moveresize_return_mask, &event);
		switch (event.type) {
			case MotionNotify:
			...
			case UnmapNotify:
				...
				aegis->eh->evUnmapDestroy(&event);
				break;
			case ConfigureRequest:
				if (event.xconfigurerequest.window != id)
					aegis->eh->evConfigureRequest(&event.xconfigurerequest);
				break;
			case MapRequest:
				map_ev = new XEvent;
				*map_ev = event;
...
bool AegisWindow::_moveOpaque(XEvent *e, int saved_w, int saved_h, int px,
	...
	if (aegis->eh->move_resize != EndMoveResizeType) return false;
	sx = nx = mnx = attrib.x;
	sy = ny = mny = attrib.y;
...
void AegisWindow::resize(XEvent *e, int how) {
	...
	aegis->eh->move_resize = ResizeType;
...
void AegisWindow::resizeOpaque(XEvent *e, int how) {
	...
	aegis->eh->move_resize = ResizeOpaqueType;
	setWindowState(wstate | StateResizeMask);
	move_resize = true;
...
void AegisWindow::moveResize(char *s) {
	...
	if (aegis->eh->move_resize != EndMoveResizeType || ! s) return;
...
void AegisWindow::toFront(void) {
	if (mergedback) {
		...
		if (focused) aegis->focusNew(id, false);
	}
}
...
void AegisWindow::windowStateCheck(bool force) {
	...
	if (force || wstate != old_wstate) {
		...
		ws->aegis->eh->evAct(NULL, frame->id, &ed);
...
void AegisWindow::addMonitor(AWindowObject *monitor, long int type) {
	...
	if (type) {
		...
		ws->aegis->eh->evAct(NULL, monitors.back()->monitor->id, &ed);
	}
}
...
void AegisWindow::signalMonitors(void) {
	...
	for (; it != monitors.end(); it++) {
		ed.type = WindowStateChangeNotify;
		ws->aegis->eh->evAct(NULL, (*it)->monitor->id, &ed);
