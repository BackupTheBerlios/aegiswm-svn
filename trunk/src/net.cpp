...
NetHandler::NetHandler(Aegis *aegis) {
	...
	aegis_net_wm_state_decor =
		XInternAtom(display, "_AEGIS_NET_WM_STATE_DECOR", false);
	aegis_net_wm_state_decortitle =
		XInternAtom(display, "_AEGIS_NET_WM_STATE_DECOR_TITLE", false);
	aegis_net_wm_state_decorborder =
		XInternAtom(display, "_AEGIS_NET_WM_STATE_DECOR_BORDER", false);
	aegis_net_wm_state_decorhandles =
		XInternAtom(display, "_AEGIS_NET_WM_STATE_DECOR_HANDLES", false);

	aegis_net_maximized_restore =
		XInternAtom(display, "_AEGIS_NET_MAXIMIZED_RESTORE", false);
	aegis_net_virtual_pos =
		XInternAtom(display, "_AEGIS_NET_VIRTUAL_POS", false);
	aegis_net_wm_desktop_mask =
		XInternAtom(display, "_AEGIS_NET_WM_DESKTOP_MASK", false);

	aegis_net_wm_merged_to =
		XInternAtom(display, "_AEGIS_NET_WM_MERGED_TO", false);
	aegis_net_wm_merged_type =
		XInternAtom(display, "_AEGIS_NET_WM_MERGED_TYPE", false);
	aegis_net_wm_merge_order =
		XInternAtom(display, "_AEGIS_NET_WM_MERGE_ORDER", false);
	aegis_net_wm_merge_atfront =
		XInternAtom(display, "_AEGIS_NET_WM_MERGE_ATFRONT", false);

	aegis_net_restart = XInternAtom(display, "_AEGIS_NET_RESTART", false);
	aegis_net_shutdown = XInternAtom(display, "_AEGIS_NET_SHUTDOWN", false);
	...
	aegis_net_event_notify =
		XInternAtom(display, "_AEGIS_NET_EVENT_NOTIFY", false);

	aegis_net_dockapp_holder =
		XInternAtom(display, "_AEGIS_NET_DOCKAPP_HOLDER", false);
	aegis_net_dockapp_prio =
		XInternAtom(display, "_AEGIS_NET_DOCKAPP_PRIO", false);

	aegis_net_cfg = XInternAtom(display, "_AEGIS_NET_CFG", false);
...
void NetHandler::getWmState(AegisWindow *ww) {
	...
	if (status == Success && items_read) {
		for (i = 0; i < items_read; i++) {
			...
			else if (data[i] == aegis_net_wm_state_decor) decor = true;
			else if (data[i] == aegis_net_wm_state_decortitle)
				decormask |= DecorTitleMask;
			else if (data[i] == aegis_net_wm_state_decorborder)
				decormask |= DecorBorderMask;
			else if (data[i] == aegis_net_wm_state_decorhandles)
				decormask |= DecorHandlesMask;
		}
...
void NetHandler::setWmState(AegisWindow *ww) {
	...
	if (ww->wstate & StateMaximizedMask) {
		...
		XChangeProperty(display, ww->id, aegis_net_maximized_restore,
				XA_CARDINAL, 32, PropModeReplace,
				(unsigned char *) data2, 6);
	} else
		XDeleteProperty(display, ww->id, aegis_net_maximized_restore);

	data[i++] = aegis_net_wm_state_decor;
	if (ww->wstate & StateDecorTitleMask)
		data[i++] = aegis_net_wm_state_decortitle;
	if (ww->wstate & StateDecorBorderMask)
		data[i++] = aegis_net_wm_state_decorborder;
	if (ww->wstate & StateDecorHandlesMask)
		data[i++] = aegis_net_wm_state_decorhandles;
...
void NetHandler::setWorkarea(AegisScreen *ws) {
	...
	XChangeProperty(aegis->display, ws->id, net_workarea, XA_CARDINAL, 32,
...
void NetHandler::wXDNDMakeAwareness(Window window) {
	...
	XChangeProperty(aegis->display, window, xdndaware, XA_ATOM,
...
void NetHandler::getXRootPMapId(AegisScreen *ws) {
	...
	if (status == Success && items_read) {
		if ((Pixmap) *data != aegis_xrootpmap_id) {
			unsigned int w, h;
			if (validate_drawable((Pixmap) (*data), &w, &h)) {

				RENDER_GET;

				if (aegis_xrootpmap_id)
					ws->config.external_bg = true;

				if (ws->bg_surface) {
					if (ws->bg_surface->pixmap != aegis_xrootpmap_id)
...
void NetHandler::setXRootPMapId(AegisScreen *ws, AegisSurface *bg_surface) {
	...
	if (old_bg) {
		if (old_bg->pixmap && old_bg->pixmap != aegis_xrootpmap_id) {
	...
	aegis_xrootpmap_id = bg_surface->pixmap;
...
void NetHandler::setDesktopMask(AegisWindow *ww) {
	...
	XChangeProperty(display, ww->id, aegis_net_wm_desktop_mask,
			XA_CARDINAL, 32, PropModeReplace,
...
void NetHandler::getDesktop(AegisWindow *ww) {
	...
	if (XGetWindowProperty(display, ww->id, aegis_net_wm_desktop_mask,
...
void NetHandler::getMergedState(AegisWindow *ww) {
	...
	if (XGetWindowProperty(display, ww->id, aegis_net_wm_merged_to, 0L,
		...
		mwin = *data;
		XFree(data);
		if (XGetWindowProperty(display, ww->id, aegis_net_wm_merged_type,
					0L, 1L, false, XA_CARDINAL, &real_type,
					&real_format, &items_read, &items_left,
					(unsigned char **) &data) == Success &&
				items_read) {
...
void NetHandler::setMergedState(AegisWindow *ww) {
	...
	if (ww->master) {
		data[0] = ww->master->id;
		XChangeProperty(display, ww->id, aegis_net_wm_merged_to,
				XA_WINDOW, 32, PropModeReplace,
				(unsigned char *) data, 1);
		data[0] = ww->mergetype;
		XChangeProperty(display, ww->id, aegis_net_wm_merged_type,
				XA_CARDINAL, 32, PropModeReplace,
				(unsigned char *) data, 1);
	} else
...
void NetHandler::setMergeOrder(AegisWindow *ww) {
	...
	if (i) {
		XChangeProperty(display, ww->id, aegis_net_wm_merge_order,
				XA_WINDOW, 32, PropModeReplace,
...
void NetHandler::getMergeOrder(AegisWindow *ww) {
	...
	status = XGetWindowProperty(display, ww->id, aegis_net_wm_merge_order,
...
void NetHandler::setMergeAtfront(AegisWindow *ww, Window win) {
	...
	XChangeProperty(display, ww->id, aegis_net_wm_merge_atfront,
			XA_WINDOW, 32, PropModeReplace,
			(unsigned char *) data, 1);
}
...
void NetHandler::getMergeAtfront(AegisWindow *ww) {
	...
	status = XGetWindowProperty(display, ww->id, aegis_net_wm_merge_atfront,
			0L, 1L, false, XA_WINDOW, &real_type,
			&real_format, &items_read, &items_left,
			(unsigned char **) &data);
...
void NetHandler::sendNotify(AWindowObject *awo, long int event,
	...
	if (aegis->running) {
		cme.message_type = aegis_net_event_notify;
...
char *NetHandler::getDockappHandler(Dockapp *dockapp) {
	...
	status = XGetWindowProperty(display, dockapp->id,
			aegis_net_dockapp_holder, 0L, 8192L,
			false, utf8_string, &real_type,
			&real_format, &items_read, &items_left,
			(unsigned char **) &data);
...
void NetHandler::setDockappHandler(Dockapp *dockapp) {
	XChangeProperty(display, dockapp->id, aegis_net_dockapp_holder,
			utf8_string, 8, PropModeReplace,
...
void NetHandler::getDockappPrio(Dockapp *dockapp) {
	...
	if (XGetWindowProperty(display, dockapp->id, aegis_net_dockapp_prio,
				0L, 1L, false, XA_CARDINAL, &real_type,
				&real_format, &items_read, &items_left,
				(unsigned char **) &data) == Success &&
			items_read) {
...
void NetHandler::setDockappPrio(Dockapp *dockapp) {
	if (dockapp->prio_set)
		XChangeProperty(display, dockapp->id, aegis_net_dockapp_prio,
				XA_CARDINAL, 32, PropModeReplace,
	...
	do {
		...
		if (status == Success && items_read) {
			if (! parser) {
				...
				parser->setFilename("_AEGIS_NET_CFG");
			}
