	...
	if (! cache_surface) {
		...
		if (crsurface) {
			...
			if (ws->aegis->client_side_rendering) {
				...
				if (ws->screen_depth == 32 && dwo->type == RootType)
					depth = 24;
...
void RenderOpText::render(DWindowObject *dwo, cairo_t *cr,
	...
	cairo_matrix_t *scale_matrix = cairo_matrix_create ();
	cairo_matrix_scale (scale_matrix, font_size, font_size);
	cairo_font_set_transform (font, scale_matrix);
	cairo_matrix_destroy (scale_matrix);
		...
		for (; sit != (*it)->strings.end(); sit++) {
			if (stroke || fill_pattern) {
				...
				if (shadow) {
					...
					cairo_move_to(cr, left_space + (*sit)->x + pos_x +
							shadow_x_offset,
							y_off + y_pos + shadow_y_offset);
