...
void Style::applyAttributes(Parser *parser, Tst<char *> *attr) {
	...
	if (value) {
		...
		if (cursorfile)
			cursor = ws->aegis->cursor->getCursor(cursorfile);
		else
#endif // XCURSOR

			cursor = ws->aegis->cursor->getCursor(value);
	}
			...
			if ((subs.find((*subinfo_it)->ident) == subs.end())) {
				...
				if (! ancestor_failure) {
					...
					if (sw->decor_root->type == WindowFrameType) {
						...
						ws->aegis->eh->evAct(NULL, sw->id, &ed);
					}
				}
			}
		}
...
void DWindowObject::renderWindow(cairo_t *cr, bool from_bgparent) {
	...
	/* ARGB visual, do real translucancy */
	...
	if (render_texture || (alpha && render_alpha)) {
		...
		if (ws->aegis->client_side_rendering) {
			pixmap = XCreatePixmap(ws->display, ws->id, surface->width,
					surface->height, ws->screen_depth);
			XImage *xim =
				XCreateImage(ws->display, ws->visual, ws->screen_depth,
	...
	if (pixmap) {
		...
		if (ws->screen_depth == 32 && type != RootType) {
			GC gc;
			XGCValues gcv;

			gcv.graphics_exposures = False;
			gc = XCreateGC (ws->display, id, GCGraphicsExposures, &gcv);

			XCopyArea (ws->display, pixmap, id, gc, 0, 0, w, h, 0, 0);
		} else {
...
SubWindowObject::SubWindowObject(DWindowObject *_parent, SubwinInfo *sinfo) :
	DWindowObject(NULL, (Window) 0, SubwindowType, NULL, NULL) {
		...
		id = XCreateWindow(ws->display, parent->id, 0, 0, 1, 1, 0,
				ws->screen_depth, CopyFromParent,
				ws->visual, create_mask, &attrib_set);
