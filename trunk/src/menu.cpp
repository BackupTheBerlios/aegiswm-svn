...
Menu::Menu(WaScreen *ws, char *n) :
	RootWindowObject(ws, 0, MenuType,
		...
		if (ws->screen_depth == 32) {
			attrib_set.background_pixel =
				WhitePixel (ws->display, ws->screen_number);
			attrib_set.border_pixel = BlackPixel (ws->display, ws->screen_number);
...
void Menu::map(AWindowObject *_awo, bool focus, bool force) {
	...
	if (ws->aegis->eh->move_resize != EndMoveResizeType) return;
	...
	if (! mapped) {
		...
		if (a_id) {
			AWindowObject *awo = (AWindowObject *)
				ws->aegis->findWin(a_id, ANY_ACTION_WINDOW_TYPE);
		...
		if (ww) {
			...
			for (; it != items.end(); ++it) {
				...
				ws->aegis->eh->evAct(NULL, (*it)->id, &ed);
		...
		if (rootitemlnk_id) {
			...
			ws->aegis->eh->evAct(NULL, rootitemlnk_id, &ed);
		}
	}
	mapped = true;
}
...
void Menu::unmap(void) {
	...
	list<MenuItem *>::iterator it = items.begin();
	for (; it != items.end(); ++it) {
		if ((*it)->focused && (*it)->id == ws->aegis->prefocus) {
			ws->aegis->focusRevertFrom(ws, (*it)->id);
			break;
		}
	}
	...
	if (was_mapped) {
		...
		if (a_id) {
			AWindowObject *awo = (AWindowObject *)
				ws->aegis->findWin(a_id, WindowType);
			if (awo) ww = awo->getWindow();
		}
...
void Menu::unmapTree(void) {
	if (rootitem_id) {
		MenuItem *rootitem = (MenuItem *)
			ws->aegis->findWin(rootitem_id, MenuItemType);
...
MenuItem::~MenuItem(void) {
	...
	ws->aegis->removeFromFocusHistory(id);
...
void MenuItem::perform(XEvent *e) {
	...
	if (a_id) {
		awo = (AWindowObject *) ws->aegis->findWin(a_id,
				ANY_ACTION_WINDOW_TYPE);
		if (! awo) return;
	} else if (menu->a_id) {
		awo = (AWindowObject *) ws->aegis->findWin(menu->a_id,
				ANY_ACTION_WINDOW_TYPE);
		if (! awo) return;
	}
...
void MenuItem::nextItem(void) {
	...
	if (next) ws->aegis->focusNew(next->id);
	else if (first) ws->aegis->focusNew(first->id);
}
...
void MenuItem::previousItem(void) {
	...
	if (prev) ws->aegis->focusNew(prev->id);
	else if (last) ws->aegis->focusNew(last->id);
}
...
void MenuItem::move(void) {
	...
	if (ws->aegis->eh->move_resize != EndMoveResizeType) return;
	ws->aegis->eh->move_resize = MoveType;
	...
	if (XGrabPointer(ws->display, id, true, ButtonReleaseMask |
		...
		ws->aegis->eh->move_resize = EndMoveResizeType;
	...
	for (;;) {
		ws->aegis->eh->eventLoop(
				&ws->aegis->eh->menu_viewport_move_return_mask, &event);
		switch (event.type) {
			case MotionNotify:
			...
			case KeyRelease:
				...
				menu->clearOutline();
				ws->aegis->eh->handleEvent(&event);
				if (ws->aegis->eh->move_resize != EndMoveResizeType) {
					menu->drawOutline(nx, ny);
					break;
				}
				menu->move(nx, ny);
		...
		switch (event.type) {
			...
			case KeyRelease:
				if (event.type == KeyPress || event.type == KeyRelease)
					event.xkey.window = id;
				ws->aegis->eh->handleEvent(&event);
				if (ws->aegis->eh->move_resize != EndMoveResizeType) break;
				while (! maprequest_list->empty()) {
					XPutBackEvent(ws->display, maprequest_list->front());
					delete maprequest_list->front();
					maprequest_list->pop_front();
