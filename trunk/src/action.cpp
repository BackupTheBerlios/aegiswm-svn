...
void Action::applyAttributes(Parser *parser, Tst<char *> *attr) {
	...
	if (value) {
		...
		if (func == &AWindowObject::setCursor) {
			...
			if (cursorfile) {
				pointer.cursor = ws->aegis->cursor->getCursor(cursorfile);
				delete [] cursorfile;
			} else
#endif // XCURSOR
	...
	if (type == ButtonPress || type == ButtonRelease) {
		...
		if (keycode) {
			detail_set = true;
			if (keycode < parser->ws->aegis->min_key ||
					keycode > parser->ws->aegis->max_key) {
	...
	for (; it != end_it; ++it) {
		if ((*it)->match(ed)) {
			...
			if (action->delay) {
				...
				ws->aegis->timer->addInterrupt(i);
			} else {
				if (action->timer_id &&
						(! ws->aegis->timer->exitsInterrupt(action->timer_id)))
					return status;
	...
	switch (type) {
		...
		case MenuItemType: {
							   AWindowObject *m_awo = (AWindowObject *)
								   ws->aegis->findWin(((MenuItem *) this)->menu->a_id,
...
void AWindowObject::doing(XEvent *, Action *a) {
	...
	ws->aegis->eh->doings.push_back(doing);
...
void AWindowObject::stopTimer(XEvent *, Action *a) {
	...
	ws->aegis->timer->removeInterrupt(timer_id);
}
...
void AWindowObject::setCursor(XEvent *, Action *a) {
	...
	else c = ws->aegis->cursor->getCursor(a->param);
